...
class RegisterFreeList(implicit p: ZnoParam) extends Module {
  val io = IO(new Bundle {
    val alc_en   = Input(Vec(p.dec_bw, Bool()))
    val alc_idx  = Output(Vec(p.dec_bw, Valid(p.Prn())))
    val num_free = Output(UInt(log2Ceil(p.prf_sz+1).W))
  })
  ...
  val frl   = RegInit(UInt(p.prf_sz.W), ~(1.U(p.prf_sz.W)))
  io.num_free := PopCount(frl)

  // Current set of free registers available to allocate
  val free_preg  = ChainedPriorityEncoderOH(frl, p.dec_bw)
  ...
  val free_mask  = (alc_idx zip free_preg).map({
    case (a, f) => f & Fill(p.prf_sz, a.valid)
  }).reduce(_|_)

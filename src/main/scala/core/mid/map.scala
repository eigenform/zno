...
class RegisterMap(implicit p: ZnoParam) extends Module {
  val rp_rs1 = IO(Vec(p.dec_bw, new MapReadPort))
  val rp_rs2 = IO(Vec(p.dec_bw, new MapReadPort))
  val wp     = IO(Vec(p.dec_bw, new MapWritePort))
  val zeroes = IO(Output(Vec(p.num_areg, Bool())))

  val regmap  = Reg(Vec(p.num_areg, p.Prn()))
  val is_zero = Reg(Vec(p.num_areg, Bool()))

  for (idx <- 0 until p.num_areg) {
